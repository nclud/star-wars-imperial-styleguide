 ...
 * INSTRUCTIONS
 *
 * Copy this script and go to https://chriszarate.github.io/bookmarkleter/ to make the bookmarklet.
 *  [x] URL-encode reserved characters: [space], %, ", <, >, #, @, &, ?
 *  [x] Wrap in an IIFE (anonymizing function).
 *  [x] Minify code using Babel Minify.
 */
...
var cacheBuster = new Date().getTime() / 1000;

var $body = document.querySelector('body');
$body.classList.add('imperial-styleguide-bookmarklet');
$body.appendChild(makeElem('<link rel="stylesheet" href="https://imperialstyleguide.com/css/bookmarklet.css?t=' + cacheBuster + '" type="text/css" media="all"></link>'));
$body.insertBefore(makeElem('<div class="neat-tv-static-screen-scan-effect"><canvas id="neat-tv-static-screen-scan-effect"></canvas></div>'), $body.firstChild);
...
document.querySelectorAll('img').forEach(function (img) {
	...
	if (width > 20) {
		var villian = villians[
			Math.floor(
				Math.random() * villians.length
			)
		];
		img.setAttribute('srcset', '');
		img.setAttribute('sizes', '');
		img.src = 'https://i0.wp.com/www.facetheforce.today/' + villian + '?resize=' + width + ',' + height;
	}
});
...
var canvas = document.querySelector("#neat-tv-static-screen-scan-effect"),
